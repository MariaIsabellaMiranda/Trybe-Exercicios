...
SELECT 
    MAX(salary) AS Salário_Maior,
    MIN(salary) AS Salário_Menor,
    SUM(salary) AS Soma_dos_Salários,
...
SELECT 
    job_id, SUM(salary) AS despesa_total
FROM
    employees
GROUP BY job_id
...
SELECT 
    job_id AS Cargos, AVG(salary) AS média_salarial
FROM
    employees
GROUP BY job_id
HAVING job_id <> 'IT_PROG'
...
SELECT 
    job_id AS Cargos,
    COUNT(employee_id) AS Total_funcionários,
    AVG(salary) AS média_salarial
FROM
    employees
GROUP BY department_id
HAVING Total_funcionários > 10;
...
SELECT 
    CONCAT(first_name, ' ', last_name) AS 'Nome Completo', DATEDIFF(NOW(), hire_date) As 'Tempo de trabalho'
FROM
    employees;
